...
Amino acids are molecules containing an amine group ($$H_2N$$-), a carboxylic acid group (-$$COOH$$), and a side chain ($$R$$) that is specific to each amino acid. The first carbon that attaches to a functional group is named alpha-carbon ($$C_\alpha$$).
